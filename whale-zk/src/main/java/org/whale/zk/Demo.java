...
public class Demo {
	...
	public static void main(String[] args) throws IOException {
		...
		try {
			...
			zkp.create("/whale-ha", "data1".getBytes(), Ids.OPEN_ACL_UNSAFE, CreateMode.EPHEMERAL);
			if (zkp.exists("/whale-ha", false) != null) {
				System.out.println("node1 exists now.");
			}
			try {
				// 当节点名已存在时再去创建它会抛出KeeperException(即使本次的ACL、CreateMode和上次的不一样)
				zkp.create("/whale-ha", "data1".getBytes(), Ids.OPEN_ACL_UNSAFE, CreateMode.PERSISTENT);
			...
			zkp = new ZooKeeper("node1:2181", TIMEOUT, null);
			// 重新建立会话后node1已经不存在了
			if (zkp.exists("/whale-ha", false) == null) {
