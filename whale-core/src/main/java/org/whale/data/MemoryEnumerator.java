...
public class MemoryEnumerator<E> implements Enumerator<E> {
	...
	public static Object convertOptiqCellValue(String strValue, String dataType) {
		...
		if ("date".equals(dataType)) {
			// convert epoch time
			/*
			 * Date dateValue = DateFormat.stringToDate(strValue); // NOTE: forces GMT
			 * timezone long millis = dateValue.getTime(); long days = millis / (1000 * 3600
			 * * 24); return Integer.valueOf((int) days); // Optiq expects Integer instead
			 * of Long. by honma
			 */
	...
	@Override
	public E current() {
		List<String> record = data.get(currentIndex);
		return rowConvert.convertRow(record, this.columnTypes);
	}
